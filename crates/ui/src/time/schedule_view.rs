...
/// Advance a slot position by one interval. Wraps to the next day at midnight.
fn advance_slot(pos: SlotPosition, interval_minutes: u32) -> SlotPosition {
    ...
    if total >= 1440 {
        SlotPosition {
            date: pos.date + chrono::Duration::days(1),
            time: NaiveTime::from_hms_opt(0, 0, 0).unwrap(),
        }
    } else {
        SlotPosition {
            date: pos.date,
            time: NaiveTime::from_hms_opt(total / 60, total % 60, 0).unwrap(),
        }
    }
}
